...
public class MainActivity extends SherlockFragmentActivity {
    ...
    public static final String serverip="http://192.168.1.120:81/";
    ...
    private ActionBar mActionBar;//页面顶端的工具条
    private SlidingMenu mMenu;//滑动menu
    ...
    protected void onCreate(Bundle savedInstanceState) 
    {
        ...
        super.onCreate(savedInstanceState);
     	...
     	try 
     	{
			context=createPackageContext("org.videolan.vlc.gui", Context.CONTEXT_IGNORE_SECURITY);
        ...
        // Set up the sliding menu//设置滑动菜单
        setContentView(R.layout.sliding_menu);;//layout/sliding_menu.xml
        mMenu = (SlidingMenu) findViewById(R.id.sliding_menu);//layout/sliding_menu.xml
        changeMenuOffset();

        View v_main = LayoutInflater.from(this).inflate(R.layout.main, null);
        mMenu.setContent(v_main);

        View sidebar = LayoutInflater.from(this).inflate(R.layout.sidebar, null);//layout/sidebar.xml 左侧弹出菜单
        final ListView listView = (ListView)sidebar.findViewById(android.R.id.list);//layout/sidebar.xml 左侧弹出菜单id
        listView.setFooterDividersEnabled(true);//显示页脚分割线？
        mSidebarAdapter = new SidebarAdapter();
        listView.setAdapter(mSidebarAdapter);
        mMenu.setMenu(sidebar);//设置滑动菜单为视图 左侧条

        /* Initialize UI variables *///初始化功能条用到的变量？
        mInfoLayout = v_main.findViewById(R.id.info_layout);//layout/main.xml，RelativeLayout，包含一个进度条ProgressBar和一个textview
        mInfoProgress = (ProgressBar) v_main.findViewById(R.id.info_progress);//layout/main.xml
        mInfoText = (TextView) v_main.findViewById(R.id.info_text);//layout/main.xml

        /* Set up the action bar */
        prepareActionBar();

        /* Set up the sidebar click listener *///设置侧拉条点击监听事件
        listView.setOnItemClickListener(new OnItemClickListener() 
        {
                    ...
                    int position, long id) 
            {
                ...
                if(current == null || current.getTag().equals(entry.id)) 
                { /* Already selected */
                    mMenu.showContent();
                    return;
                }
       ...
       ///////////////获得机器码
			  ...
			  Log.e(TAG, "获取机器码");
				 ...
				 try { 
					...
					// identity国际移动用户号码标识，这个一般大家是不知道，GSM必须写在卡内相关文件中；
					// MSISDN:mobile subscriber ISDN用户号码，这个是我们说的139，136那个号码；
					// ICCID:ICCidentity集成电路卡标识，这个是唯一标识一张卡片物理号码的；
					// IMEI：international mobile Equipment identity手机唯一标识码；

					String tmmsi =tm.getSubscriberId();     //取出IMSI
					String tmDevice = "" + tm.getDeviceId();// 设备的机器码
					String tmtel = tm.getLine1Number();     //取出MSISDN，很可能为空,即手机号
					String tmSerial = "" + tm.getSimSerialNumber();// 取出ICCID,SIM卡序列号
					String androidId = ""
							+ android.provider.Settings.Secure.getString(
									getContentResolver(),
									android.provider.Settings.Secure.ANDROID_ID);// DeviceID
					final UUID deviceUuid = new UUID(androidId.hashCode(),
							((long) tmDevice.hashCode() << 32) | tmSerial.hashCode());
					String uniqueId = deviceUuid.toString();
				 Log.e(TAG, "加密前ID(DeviceID/androidId)："+androidId);
				 Log.e(TAG, "加密后ID："+uniqueId);
				 DeviceInfo=uniqueId;
    ...
    //获取授权码
	public static String loadSharedPreferences(String videoid)
	{
	...
	//存储授权码
	public  static void saveSharedPreferences(String videoid,String sn)
	{
		SharedPreferences sharedPreferences= context.getSharedPreferences
				(PreferenceFileName,MODE);
    ...
    private void prepareActionBar() {
        ...
        /* Add padding between the home button and the arrow */  //工具条上的vlc logo
        ImageView home = (ImageView)findViewById(Util.isHoneycombOrLater()
                ? android.R.id.home : R.id.abs__home);
        if (home != null)
        //	home.setBackgroundColor(145);
     ...
     */
    @Override
    public boolean onCreateOptionsMenu(Menu menu) { //创建工具栏上的菜单
        ...
        inflater.inflate(R.menu.media_library3, menu);
    ...
    /**
     * Handle onClick form menu buttons// 工具栏上的菜单点击事件
     */
    @Override
    ...
    public void sendHandleMessage(String btn)
    {
    	dirname=btn;
    	vurl=serverip+btn+"/";
    ...
    public void DailingButtonClicked(View view)
    {
    	String number = this.getString(R.string.callme);
    ...
    public static void playclassvideo(String vurl) 
    {
    	AudioServiceController c = AudioServiceController.getInstance();
        String s =vurl;
        c.load(s, false);
    ...
    public class RefreshHandler extends Handler
    {
        ...
        public void handleMessage(Message msg) 
        {
         try
         	{
        	 ((FragmentActivity) MainActivity.mContext).getSupportFragmentManager().popBackStack(null, FragmentManager.POP_BACK_STACK_INCLUSIVE);
             Fragment current = ((MainActivity) MainActivity.mContext).getSupportFragmentManager().findFragmentById(R.id.fragment_placeholder);
             ((MainActivity) MainActivity.mContext).getSupportFragmentManager().popBackStack(null, FragmentManager.POP_BACK_STACK_INCLUSIVE);
             FragmentTransaction ft = ((MainActivity) MainActivity.mContext).getSupportFragmentManager().beginTransaction();
             ft.replace(R.id.fragment_placeholder,  ((MainActivity) MainActivity.mContext).getFragment("netclass"), "netclass");
             ((MainActivity) MainActivity.mContext).mCurrentFragment = "netclass";
             
             ft.commit();
        	 //playclassvideo("http://192.168.1.120:81/vview/j001/1.mp4");
         	}
         catch(Exception e)
         	{
        	 e.printStackTrace();
